"""OMLX provider — local models via an OpenAI-compatible HTTP API.
...
# See orb/llm/ollama.py for rationale — split timeouts surface stalls
# (e.g. a model listed by /v1/models but not actually loaded) in minutes
...
class OmlxProvider(LLMClient):
    """OMLX local-model provider using an OpenAI-compatible API surface."""
    ...
    async def complete(
        ...
        *,
        ...
        if not model_id:
            raise RuntimeError("No configured OMLX model available")
        ...
        if resp.status_code >= 400:
            raise Exception(f"OMLX {resp.status_code}: {resp.text[:500]}")
        data = resp.json()
        ...
        logger.info(
            "OMLX completion model=%s base_url=%s messages=%d prompt_tokens=%d output_tokens=%d tools=%d",
            model,
            self._base_url,
            len(messages),
