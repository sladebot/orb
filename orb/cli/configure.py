"""Interactive `orb configure` flow.

Walks the user through: pick provider → auth (if cloud) → refresh catalog
→ toggle enabled models → pick default model per tier → save to
...
def _pick_provider() -> str | None:
    ...
    while True:
        ...
        if lowered in {"q", "quit", "exit"}:
            return None
        if not raw:
            # Empty → treat as quit so automation can end the loop cleanly.
            return None
        if raw.isdigit():
            idx = int(raw) - 1
...
async def _refresh_catalog() -> None:
    ...
    Imports live inside the function so this module stays light for tests
...
def _toggle_models(provider: str, catalog: list[dict[str, Any]], cfg: dict[str, Any]) -> dict[str, dict[str, bool]]:
...
async def configure_provider(provider: str) -> None:
    ...
    if not await _ensure_authed(provider):
        print(f"Skipped {provider}: not authenticated.")
        return
...
async def run_configure() -> None:
    print("Orb configure — interactive provider / model setup")
    while True:
        cfg = config_cli.load_config()
