...
def test_build_providers_local_only_keeps_omlx_when_enabled(monkeypatch):
    monkeypatch.setattr("orb.cli.config.local_models_enabled", lambda: True)
    monkeypatch.setattr(
        "orb.cli.config.get",
        lambda key: {"ollama": {"enabled": False}, "vmlx": {"enabled": False}, "omlx": {"enabled": True, "base_url": "http://localhost:8000/v1"}} if key == "providers" else None,
    )
    monkeypatch.setattr(registry, "_omlx_base_url", lambda: "http://localhost:8000/v1")

    class FakeOmlxProvider:
        def __init__(self, base_url: str, api_key=None):
            self.base_url = base_url
            self.api_key = api_key

    monkeypatch.setattr("orb.llm.registry.OmlxProvider", FakeOmlxProvider, raising=False)
    monkeypatch.setattr("orb.llm.omlx.OmlxProvider", FakeOmlxProvider)
...
@pytest.mark.asyncio
async def test_fetch_omlx_catalog_uses_registry_base_url(monkeypatch):
    requested = {}
    ...
    monkeypatch.setattr("httpx.AsyncClient", FakeAsyncClient)
    monkeypatch.setattr(registry, "_omlx_base_url", lambda: "http://remote-omlx.example:8000/v1")
    monkeypatch.setattr(
        "orb.runtime.graph_runtime.get_config",
        lambda key: {"omlx": {}} if key == "providers" else None,
    )

    runtime = GraphRuntime()
    catalog, defaults = await runtime._fetch_omlx_catalog()

    assert requested["url"] == "http://remote-omlx.example:8000/v1/models"
    assert catalog == [{"id": "Qwen/Qwen2.5-Coder-7B-Instruct", "label": "Qwen/Qwen2.5-Coder-7B-Instruct", "local": True}]
    assert defaults["local_small"] == "Qwen/Qwen2.5-Coder-7B-Instruct"
