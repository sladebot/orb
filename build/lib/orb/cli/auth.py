...
async def auth_anthropic(credential: str | None = None) -> None:
    """Store Anthropic auth, guiding users through Claude setup-token."""
    ...
    print(
        ...
        "  2. Copy the generated setup-token.\n"
        "  3. Paste it here.\n\n"
        "You can also paste an Anthropic API key instead.\n"
    )
    try:
        raw = input("Anthropic credential: ").strip()
    except (EOFError, KeyboardInterrupt):
        print("\nAborted.")
        return
    if not raw:
        print("No Anthropic credential provided.")
        return
...
async def auth_status() -> None:
    ...
    if stored_creds.get("oauth_token"):
        ...
        ant_source = "ANTHROPIC_OAUTH_TOKEN env"
    else:
        ant_source = "ANTHROPIC_API_KEY env"
    if ant_key:
        tasks["anthropic"] = (f"  anthropic  {ant_source}  (****{ant_key[-4:]})", asyncio.create_task(_check_anthropic(ant_key)))
    else:
        print("  anthropic  not authenticated  (run: orb auth anthropic)")
